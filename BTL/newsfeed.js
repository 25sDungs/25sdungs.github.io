window.onload = function () {
  ...
  $(document).ready(() => {
    ...
    $(document).ready(()=>{
      ...
      $(".user-chat").click(()=>{
        $(".pop-up").show().addClass("animate__animated animate__zoomIn");
      })
    })
  });
...
fetch('user-contact.json')
  ...
  .catch(error => console.error('Error fetching user data:', error));
fetch('nofication.json')
  .then(response => response.json())
  .then(data => {
    const notifList = document.querySelector(".menu-nof");
    data.forEach(user => {
